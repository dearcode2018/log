...
public final class PersonControllerTest extends BaseTest {
	...
	public void testGet() {
		try {
			...
			requestBuilder.header("accessCode", "100002");
			// mvc 结果
	...
	public void testPostJson() {
		try {
			...
			//requestBuilder.contentType(MediaType.APPLICATION_FORM_URLENCODED_VALUE);
			requestBuilder.contentType(MediaType.APPLICATION_JSON_VALUE);
			requestBuilder.content("{\"name\":\"admin\", \"password\":\"123456\", \"password2\":\"123456\"}");
			requestBuilder.header("accessCode", "100003");
			// 模拟 mvc 对象，设置 WebApplicationContext，然后构建 模拟mvc对象
			MockMvc mockMvc = MockMvcBuilders.webAppContextSetup(webApplicationContext).build(); 
			// mvc 结果
			MvcResult mvcResult = mockMvc.perform(requestBuilder).andReturn();
	...
	public void testPostXml() {
		try {
			...
			requestBuilder.content("<person><name>admin</name><password>123456</password/></person>");
			// 模拟 mvc 对象，设置 WebApplicationContext，然后构建 模拟mvc对象
			MockMvc mockMvc = MockMvcBuilders.webAppContextSetup(webApplicationContext).build(); 
			// mvc 结果
			MvcResult mvcResult = mockMvc.perform(requestBuilder).andReturn();
